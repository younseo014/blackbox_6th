...
- 세션 종료 시 등록된 업무 시간대, 매장 구역, 이전에 학습한 스켈레톤 유사도를
  함께 사용해 업무 후보를 만듭니다. 신뢰도가 낮거나 상위 후보 차이가 작은
  구간은 기준선 학습에 바로 넣지 않고 개발자 모드의 검토 대기함으로 보냅니다.
- 개발자는 해당 행동 구간만 스켈레톤으로 재생하고, `업무 맥락 등록`에 있는
  이름 중 하나를 선택해 확정하거나 `판별 불가`로 제외할 수 있습니다.
- 수동 확정된 학습 모드 구간은 업무별 기준선과 동작 유사도 표본에 함께
  반영되어, 같은 패턴이 반복될수록 자동 라벨링으로 전환됩니다.
