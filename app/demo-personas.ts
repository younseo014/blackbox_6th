...
const PERSONA_DEFINITIONS: DemoPersonaDefinition[] = [
  // -- Persona: 안정적인 한 주 (none) --------------------------------------
  {
    ...
    summary: "혼자 편의점을 안정적으로 운영합니다. 이번 주는 반복 확인과 업무 지연이 모두 평소 범위 안에서 유지되는 가장 흔한 한 주를 가정했습니다.",
    weekRange: "8월 4일–10일",
    week: [
      ...
      { day: "수", date: "8/6", safetyAlerts: 0, doubleChecks: 1, unfinishedTasks: 0, microDelayRate: 8, busy: false, note: "재고 정리 후 평소처럼 마무리", examples: [{ label: "16:40 재고 확인", motionType: "normal_task" }, { label: "18:10 진열대 정리", motionType: "high_reach" }, { label: "19:01 마감 확인 1회", motionType: "normal_task" }] },
    ...
    insights: [
      { icon: "≈", kicker: "1. 평소 대비 변화", title: "이번 주는 평소 범위 안이에요", body: "반복 확인과 업무 지연 모두 지난 흐름과 비슷합니다. 메모리 가드는 대부분의 주가 이렇게 조용히 지나간다는 것도 함께 보여드리고 싶었어요." },
  ...
  // -- Persona: 점진적인 변화 (notable) -------------------------------------
  {
    ...
    summary: "평소에는 혼자 카페를 안정적으로 운영합니다. 이번 주 후반에는 반복 확인, 업무 중단, 처리 지연이 함께 늘어나는 패턴을 가정했습니다.",
    weekRange: "8월 4일–10일",
    week: [
      ...
      { day: "목", date: "8/7", safetyAlerts: 0, doubleChecks: 1, unfinishedTasks: 0, microDelayRate: 9, busy: false, note: "업무 확인을 한 차례 반복", examples: [{ label: "14:10 주문 대기열 증가", motionType: "queue_shift" }, { label: "19:09 마감 항목 재확인", motionType: "double_check" }] },
      ...
      { day: "토", date: "8/9", safetyAlerts: 0, doubleChecks: 2, unfinishedTasks: 2, microDelayRate: 45, busy: false, note: "반복 확인과 미완료 업무가 크게 늘어난 토요일", examples: [{ label: "15:02 주문 입력 후 9분 지연", motionType: "micro_delay" }, { label: "16:20 결제 업무 재시도", motionType: "register_tap" }, { label: "17:52 재고 박스 하단 확인", motionType: "low_bend" }, { label: "19:28 마감 항목 재확인", motionType: "double_check" }] },
    ...
    insights: [
      { icon: "↗", kicker: "1. 평소 대비 변화", title: "주 후반에 신호가 겹쳐요", body: "토·일 이틀 동안 반복 확인, 미완료 업무, 처리 지연이 함께 늘었습니다. 한 가지 실수보다 여러 지표가 같은 기간에 함께 움직이는지를 봅니다." },
  ...
  // -- Persona: 성수기 혼잡 (confound note) ---------------------------------
  {
    ...
    summary: "평소엔 안정적이지만, 크리스마스 성수기 주말에는 예약 주문이 몰려 반복 확인과 업무 지연이 함께 늘어나는 패턴을 가정했습니다. 두 날 모두 '바쁨'으로 직접 표시했습니다.",
    ...
    insights: [
      { icon: "☂", kicker: "1. 숫자만 보면 놀랄 수 있어요", title: "하지만 이 이틀은 '바쁨'으로 직접 표시된 날이에요", body: "반복 확인과 업무 지연이 눈에 띄게 늘었지만, 사장님이 오늘 매장 분위기를 직접 '바쁨'으로 표시한 날들이라 손님이 몰려 생긴 변화일 가능성이 큽니다." },
  ...
  // -- Persona: 하루의 소란 (single-day noise, watch not notable) ----------
  {
    ...
    summary: "오랫동안 안정적으로 가게를 운영해왔습니다. 토요일 하루만 처리 지연이 늘고, 다음 날 곧바로 평소 흐름으로 돌아오는 '하루짜리 소란'을 가정했습니다.",
    weekRange: "8월 4일–10일",
    week: [
      ...
      { day: "토", date: "8/9", safetyAlerts: 0, doubleChecks: 1, unfinishedTasks: 0, microDelayRate: 45, busy: false, note: "오후 업무가 한 차례 평소보다 오래 걸린 하루", examples: [{ label: "14:20 재고 정리 중 처리 지연", motionType: "micro_delay" }, { label: "14:45 주문 응대 후 정리", motionType: "queue_shift" }, { label: "19:05 마감 확인 1회", motionType: "normal_task" }] },
    ...
    insights: [
      { icon: "•", kicker: "1. 하루의 소란, 패턴은 아니에요", title: "토요일 딱 하루만 튀었어요", body: "토요일에 처리 지연이 늘었지만, 다음 날인 일요일에는 곧바로 평소 수준으로 돌아왔습니다. 메모리 가드는 하루의 소란만으로 알리지 않고, 며칠 더 이어지는지 먼저 봐요." },
      { icon: "⌁", kicker: "2. 시간축으로 원인 찾기", title: "타임라인을 보면 이유를 알 수 있어요", body: "타임라인을 되짚어보면 재고 정리 중 잠시 업무가 지연된 날이었다는 걸 확인할 수 있어요. 원인을 알 수 있는 사건은 지속적인 변화와 구분돼요." },
...
// actions - recordDoubleCheck on a same-day re-check and a task crossing
// SLOW_DELAY_SECONDS - feed the same counted signals; see page.tsx's
// stopPoseTracking.)

const CATEGORY_BY_DETECTED_TYPE: Record<
  DetectedMotionEvent["type"],
  "doubleChecks" | "microDelay"
> = {
  double_check: "doubleChecks",
  micro_delay: "microDelay",
};

const RULE_BY_CATEGORY: Record<"doubleChecks" | "microDelay", string> = {
  doubleChecks:
...
const SIGNAL_REASON_BY_CATEGORY: Record<"doubleChecks" | "microDelay", string> = {
...
export function explainDemoEvent(
  ...
  if (detected.length === 0) {
    return {
      rule: "이 동작에서는 반복 확인이나 업무 지연에 해당하는 패턴이 감지되지 않았어요. 케어 리포트에 따로 집계되지 않는 일반 업무 장면이에요.",
      detected: [],
      metricLine: null,
